...
async def film_main(
    ...
    return [FilmMain(uuid=x.id, title=x.title, imdb_rating=x.imdb_rating, description=x.description) for x in films_all_fields]
...
async def film_search(
    ...
    return [FilmMain(uuid=x.id, title=x.title, imdb_rating=x.imdb_rating) for x in films_all_fields_search]  # type: ignore
...
async def film_details(film_id: str, film_service: FilmService = Depends(get_film_service)) -> FilmDetail:
    ...
    return FilmDetail(uuid=film.id, title=film.title, imdb_rating=film.imdb_rating, description=film.description,  # type: ignore
                      genre=film.genre, actors=film.actors, writers=film.writers, director=film.director)  # type: ignore
